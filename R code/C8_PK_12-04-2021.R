...
### sigma = 100 * Ct in the likelihood in formula (14) 
...
################################################# R code translated from Matlab code: ExConc_water_BR6_savitzonly_geo_2_work.m by Yachen Zhu ###################################################
...
for( q in 1:l ){
  ...
  wts[, q] <-  -1/2*(EC_IN_BR4$C8.measure - C8.est[, 8, q])^2 / (100*EC_IN_BR4$C8.measure)^2  
...
C8.est.abc.1 <- C8.est[ , , exp(wt) != 0]  # acceptance rate = 99.1%
dim(C8.est.abc.1)
C8.est.abc <- C8.est[ , , exp(wt) >= 1e-300]  # acceptance rate = 99.0%
dim(C8.est.abc)
...
mean((C8.test$C8.measure - C8.test$C8.est.abc)^2) # 10640.83
mean(abs(C8.test$C8.measure - C8.test$C8.est.abc)) # 36.86

y1 <- y %>% left_join(C8.long %>% select(-preg), by = c("participantid", "Exposure_yr")) %>% distinct()
cor(y1$C8_estimate, y1$C8.est.abc, method = "spearman") # 0.41
...
for(i in 1:dim(C8.est.abc)[3]){    # 990
