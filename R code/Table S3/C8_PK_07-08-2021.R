...
### sigma = 5 * Ct in the likelihood in formula (14) 
...
for( q in 1:l ){
  ...
  wts[, q] <-  -1/2*(EC_IN_BR4$C8.measure - C8.est[, 8, q])^2 / (5*EC_IN_BR4$C8.measure)^2  
...
C8.est.abc.1 <- C8.est[ , , exp(wt) != 0]  # acceptance rate = 37.3%
dim(C8.est.abc.1)
C8.est.abc <- C8.est[ , , exp(wt) >= 1e-300]  # acceptance rate = 35.7%
dim(C8.est.abc)
...
mean((C8.test$C8.measure - C8.test$C8.est.abc)^2)  # 7493.59
mean(abs(C8.test$C8.measure - C8.test$C8.est.abc)) # 27.95

y1 <- y %>% left_join(C8.long %>% select(-preg), by = c("participantid", "Exposure_yr")) %>% distinct()
cor(y1$C8_estimate, y1$C8.est.abc, method = "spearman") # 0.33
...
for(i in 1:dim(C8.est.abc)[3]){     # 357
