...
C8.est.abc.1 <- C8.est[ , , exp(wt) != 0]  # acceptance rate = 46.1%
dim(C8.est.abc.1)
C8.est.abc <- C8.est[ , , exp(wt) >= 1e-300]  # acceptance rate = 43.5%
dim(C8.est.abc)
...
mean((C8.test$C8.measure - C8.test$C8.est.abc)^2)  # 7037.42
...
cor(y1$C8_estimate, y1$C8.est.abc, method = "spearman") # 0.35
...
for(i in 1:dim(C8.est.abc)[3]){     # 435
