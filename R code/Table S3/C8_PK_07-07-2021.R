...
##### Assumptions
### short-term shrinkage water ingestion rate
### Autocorrelation factor = 0.75
### sigma = 4 * Ct in the likelihood in formula (14) 
...
sum(is.na(data1$R0))  # 2462, 40%  # 39699/68241=58%
sum(is.na(data1$WT.kg))  # 0


### water consumption rate by age group - page 13 EPA Exposure Factors Handbook 2019 (unit: L/day)
Age <- c("1 to < 2", "2 to < 3", "3 to < 6", "6 to < 11", "11 to < 16", "16 to < 21", 
         "21 to <50", "50 to <60", "60 to <70", "70 to <80")
# mean of water consumption rate (Consumers-Only) 
mean <- 0.001 * c(245, 332, 338, 455, 562, 722, 1277, 1419, 1394, 1214)
# 95th percentile (unit: L/day)
ninetyfive.p <- 0.001 * c(658, 901, 836, 1258, 1761, 2214, 3353, 3388, 3187, 2641) 
...
#conc_Belpre[1,] <- as.character(conc_Belpre[1,])
...
library(ggplot2)
tiff("~/Desktop/PK Bayesian/plots/PFOAWaterConcentrations.tiff", width = 10, height = 6, units = 'in', res = 500)
ggplot(data = conc_all_log, aes(x = Year, y = `log concentration`, color = `Water District`)) + geom_line() + ylim(-3, 1) +
  ylab("Concentrationn in log 10 (ppb)")
dev.off()
...
for( q in 1:l ){
  ...
  # unit: ug/L * L = ug
  Dose <- (FR * EC_IN_BR5 * qlnorm(p, table[1, 5], table[1, 6]) * 365.25) * (FR1 >= 1 & FR1 < 2) +  # 1-2 years old
    (FR * EC_IN_BR5 * qlnorm(p, table[2, 5], table[2, 6]) * 365.25) * (FR1 >= 2 & FR1 < 3) +  # 2-3 years old
    (FR * EC_IN_BR5 * qlnorm(p, table[3, 5], table[3, 6]) * 365.25) * (FR1 >= 3 & FR1 < 6) +  # 3-6 years old
    (FR * EC_IN_BR5 * qlnorm(p, table[4, 5], table[4, 6]) * 365.25) * (FR1 >= 6 & FR1 < 11) +  # 6-11 years old
    (FR * EC_IN_BR5 * qlnorm(p, table[5, 5], table[5, 6]) * 365.25) * (FR1 >= 11 & FR1 < 16) +  # 11-16 years old
    (FR * EC_IN_BR5 * qlnorm(p, table[6, 5], table[6, 6]) * 365.25) * (FR1 >= 16 & FR1 < 21) +  # 16-21 years old
    (FR * EC_IN_BR5 * qlnorm(p, table[7, 5], table[7, 6]) * 365.25) * (FR1 >= 21 & FR1 < 50) +  # 21-50 years old
    (FR * EC_IN_BR5 * qlnorm(p, table[8, 5], table[8, 6]) * 365.25) * (FR1 >= 50 & FR1 < 60) +  # 50-60 years old
    (FR * EC_IN_BR5 * qlnorm(p, table[9, 5], table[9, 6]) * 365.25) * (FR1 >= 60 & FR1 < 70) +  # 60-70 years old
    (FR * EC_IN_BR5 * qlnorm(p, table[10, 5], table[10, 6]) * 365.25) * (FR1 >= 70)  # 70-80 years old, no participants older than 80 at the time of blood sampling
  ...
  wts[, q] <-  -1/2*(EC_IN_BR4$C8.measure - C8.est[, 8, q])^2 / (4*EC_IN_BR4$C8.measure)^2  
...
C8.est.abc.1 <- C8.est[ , , exp(wt) != 0]  # acceptance rate = 28.6%
dim(C8.est.abc.1)
C8.est.abc <- C8.est[ , , exp(wt) >= 1e-300]  # acceptance rate = 27.1%
dim(C8.est.abc)
...
cor(C8.test$C8.est.abc, C8.test$C8.measure) # 0.65
cor(C8.test$C8.est.abc, C8.test$C8.measure, method = "spearman") # 0.68
mean((C8.test$C8.measure - C8.test$C8.est.abc)^2)  # 8048.46
mean(abs(C8.test$C8.measure - C8.test$C8.est.abc)) # 28.89

y1 <- y %>% left_join(C8.long %>% select(-preg), by = c("participantid", "Exposure_yr")) %>% distinct()
cor(y1$C8_estimate, y1$C8.est.abc, method = "spearman") # 0.32
...
for(i in 1:dim(C8.est.abc)[3]){     # 271
