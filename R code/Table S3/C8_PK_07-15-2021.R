...
### sigma = 3 * Ct in the likelihood in formula (14) 
...
for( q in 1:l ){
  ...
  wts[, q] <-  -1/2*(EC_IN_BR4$C8.measure - C8.est[, 8, q])^2 / (3*EC_IN_BR4$C8.measure)^2  
...
C8.est.abc.1 <- C8.est[ , , exp(wt) != 0]  # acceptance rate = 15.6%
dim(C8.est.abc.1)
C8.est.abc <- C8.est[ , , exp(wt) >= 1e-300]  # acceptance rate = 14.1%
dim(C8.est.abc)
...
cor(C8.test$C8.est.abc, C8.test$C8.measure) # 0.64
cor(C8.test$C8.est.abc, C8.test$C8.measure, method = "spearman") # 0.68
mean((C8.test$C8.measure - C8.test$C8.est.abc)^2) # 8975.16
mean(abs(C8.test$C8.measure - C8.test$C8.est.abc)) # 30.81

y1 <- y %>% left_join(C8.long %>% select(-preg), by = c("participantid", "Exposure_yr")) %>% distinct()
cor(y1$C8_estimate, y1$C8.est.abc, method = "spearman") # 0.31
...
for(i in 1:dim(C8.est.abc)[3]){     # 141
