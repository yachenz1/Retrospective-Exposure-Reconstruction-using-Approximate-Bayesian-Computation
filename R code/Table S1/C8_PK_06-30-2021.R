...
L <- 58
corr <- toeplitz(0.5^(0:(L-1)))   # AR(1) correlation structure
sigma <- matrix(0, L, L)
...
C8.est.abc.1 <- C8.est[ , , exp(wt) != 0]  # acceptance rate = 20.2%
dim(C8.est.abc.1)
C8.est.abc <- C8.est[ , , exp(wt) >= 1e-300]  # acceptance rate = 18.8%
dim(C8.est.abc)
...
mean((C8.test$C8.measure - C8.test$C8.est.abc)^2)  # 7385.55
mean(abs(C8.test$C8.measure - C8.test$C8.est.abc))  # 27.70
...
for(i in 1:dim(C8.est.abc)[3]){   # 188
