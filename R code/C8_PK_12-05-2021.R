...
### sigma = 10 * Ct in the likelihood in formula (14) 
...
for( q in 1:l ){
  ...
  wts[, q] <-  -1/2*(EC_IN_BR4$C8.measure - C8.est[, 8, q])^2 / (10*EC_IN_BR4$C8.measure)^2  
...
C8.est.abc.1 <- C8.est[ , , exp(wt) != 0]  # acceptance rate = 70%
dim(C8.est.abc.1)
C8.est.abc <- C8.est[ , , exp(wt) >= 1e-300]  # acceptance rate = 68.4%
dim(C8.est.abc)
...
mean((C8.test$C8.measure - C8.test$C8.est.abc)^2) # 6234.05
...
cor(y1$C8_estimate, y1$C8.est.abc, method = "spearman") # 0.37
...
for(i in 1:dim(C8.est.abc)[3]){    # 684
