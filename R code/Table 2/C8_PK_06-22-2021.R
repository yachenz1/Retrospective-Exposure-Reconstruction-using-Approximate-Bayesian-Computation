#################################################### Original Estimates/Fixed Values of Models Parameters #######################################################
######################################################################### Date: 06/30/2021 ######################################################################
...
UTM <- read.table("~/Desktop/PK Bayesian/data/Zip_unique.csv", header = F, sep = ",")
...
library(readxl)
XI <- read_excel("~/Desktop/PK Bayesian/data/cellcolumn.xls", col_names = F)
YI <- read_excel("~/Desktop/PK Bayesian/data/cellrow.xls", col_names = F)
...
Percentpublic <- read.csv("~/Desktop/PK Bayesian/Pipe Networks/Percentpublic_new.csv", header = F)
...
vol <- 0.2366  # L/day, for one 8oz cup
...
# Age-specifc body weight for the participants based on the EPA Exposure Factors Handbook
for(i in 1:nrow(BW)){
  for(j in 2:ncol(BW)){
...
conc_Belpre[2, ] <- ifelse(conc_Belpre[1, ] > GAC_BL - 1, 0, conc_Belpre[2, ])

conc_LittleHocking[2, ] = 0 * ( conc_LittleHocking[1, ] > GAC_LH ) + 
  conc_LittleHocking[2, ] * 10 / 12 * ( conc_LittleHocking[1, ] == GAC_LH ) +  # accounting for 10 months of not using GAC filter
  conc_LittleHocking[2, ] * ( conc_LittleHocking[1, ] < GAC_LH )

conc_Lubeck[2, ] = 0 * ( conc_Lubeck[1, ] > GAC_LB ) + 
  conc_Lubeck[2, ] * 5/12 * (conc_Lubeck[1, ] == GAC_LB ) +  # accounting for 5 months of not using GAC filter
  conc_Lubeck[2, ] * (conc_Lubeck[1, ] < GAC_LB )

conc_TuppersPlains[2, ] = ifelse( conc_TuppersPlains[1, ] > GAC_TP - 1, 0, conc_TuppersPlains[2, ] )

conc_Mason[2, ] = ifelse(conc_Mason[1, ] > GAC_MC - 1, 0, conc_Mason[2, ]) 

conc_Pomeroy[2, ] = ifelse(conc_Pomeroy[1, ] > GAC_PO - 1, 0, conc_Pomeroy[2, ])
...
for ( k in 1:nrow(ED_BR) ) {
  for( m in 1:ncol(conc_Belpre) ) {
    ...
    EC_IN_BR[k, m] = 0 * (ED_BR[k, 2] > conc_Belpre[1, m] |
      ...
      conc_Belpre[2, m] * ( ED_BR[k, 4] == 1 ) + 
      conc_TuppersPlains[2, m] * ( ED_BR[k, 4] == 2 ) + 
      conc_LittleHocking[2, m] * ( ED_BR[k, 4] == 3 ) + 
      conc_Lubeck[2, m] * ( ED_BR[k, 4] == 4 ) + 
      conc_Mason[2, m] * ( ED_BR[k, 4] == 5 ) + 
      conc_Pomeroy[2, m] * ( ED_BR[k, 4] == 6 ) 
...
for(index in 1:years_run_max){
  ...
  file_to_open9 <- paste0('~/Desktop/PK Bayesian/data/mt3d10_csv/', mt_file)
...
# unit: ug/L * L = ug
Dose <- (FR * EC_IN_BR5 * rep(table[1, 2], nrow(Dose)) * 365.25) * (FR1 >= 0 & FR1 < 2) +  # 0-2 years old
  (FR * EC_IN_BR5 * rep(table[2, 2], nrow(Dose)) * 365.25) * (FR1 >= 2 & FR1 < 16) +  # 2-16 years old
  (FR * EC_IN_BR5 * rep(table[3, 2], nrow(Dose)) * 365.25) * (FR1 >= 16 & FR1 < 21) +  # 16-21 years old
  (FR * EC_IN_BR5 * rep(table[4, 2], nrow(Dose)) * 365.25) * (FR1 >= 21 & FR1 < 50) +  # 21-50 years old
  (FR * EC_IN_BR5 * rep(table[5, 2], nrow(Dose)) * 365.25) * (FR1 >= 50)  # 50-60 years old
...
for(j in 1:58){
  Dose[, j] <- ifelse(!is.na(EC_IN_BR4[, 69]) & (EC_IN_BR4[, 69] %in% c("1", "2", "3", "4", "5", "6")),
                      EC_IN_BR5[, j] * 0.5 * vol * 365.25 * (!is.na(EC_IN_BR4[, 69]) & EC_IN_BR4[, 69] == "1") +   # R0 = 1
                        EC_IN_BR5[, j] * 1.5 * vol * 365.25 * (!is.na(EC_IN_BR4[, 69]) & EC_IN_BR4[, 69] == "2") +   # R0 = 2
                        EC_IN_BR5[, j] * 3.5 * vol * 365.25 * (!is.na(EC_IN_BR4[, 69]) & EC_IN_BR4[, 69] == "3") +   # R0 = 3
                        EC_IN_BR5[, j] * 5.5 * vol * 365.25 * (!is.na(EC_IN_BR4[, 69]) & EC_IN_BR4[, 69] == "4") +   # R0 = 4
                        EC_IN_BR5[, j] * 7.5 * vol * 365.25 * (!is.na(EC_IN_BR4[, 69]) & EC_IN_BR4[, 69] == "5") +   # R0 = 5
                        EC_IN_BR5[, j] * 9 * vol * 365.25 * (!is.na(EC_IN_BR4[, 69]) & EC_IN_BR4[, 69] == "6"),   # R0 = 6
                      Dose[, j])
}
...
C8.est <- cbind( pregnant.yrs$ID, apply( W[, 1:58, 1] * Dose, 1, sum ), apply( W[, 1:58, 2] * Dose, 1, sum ), 
...
for (i in 1:nrow(W)) {
  for (r in 1:7){
    if(!is.na(pregnant.yrs[i, r+1])){
      C8.est[i, r+1] <- (C8.est[i, r+1] + 0.11 * pregnant.yrs[i, r+1]) * (pregnant.yrs[i, r+1] < 1999) +  # Year < 1999
        (C8.est[i, r+1] + 5.2 - 0.33*(pregnant.yrs[i, r+1] - 1999)) * (pregnant.yrs[i, r+1] >= 1999)  # Year >= 1999
    }
  }
}
...
pregnant.yrs.long <- gather(pregnant.yrs, key = "preg", value = "year", -ID)
C8.est.long <- gather(data.frame(C8.est), key = "preg", value = "C8.est", -ID)
C8.long <- pregnant.yrs.long %>% 
  left_join(C8.est.long, by = c("ID", "preg")) %>% 
  filter(!is.na(year)) %>% 
...
cor(C8.test$C8.est, C8.test$C8.measure) # 0.62
cor(C8.test$C8.est, C8.test$C8.measure, method = "spearman") # 0.67
mean((C8.test$C8.measure - C8.test$C8.est)^2) # 22384
mean(abs(C8.test$C8.measure - C8.test$C8.est)) # 48.86

y1 <- y %>% left_join(C8.long %>% select(-preg), by = c("participantid", "Exposure_yr")) %>% distinct()
cor(y1$C8_estimate, y1$C8.est, method = "spearman") # 0.42
